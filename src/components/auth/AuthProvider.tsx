...
export function AuthProvider({children}:{children:React.ReactNode}){
 ...
 const login=async(email:string,password:string)=>{try{const r=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password})});const d=await r.json();if(!r.ok)return {ok:false,error:d.error||'Login gagal.'};setUser(d.user);await initializePersistence();return {ok:true,redirect:roleDashboard[d.user.role as Role]}}catch{return {ok:false,error:'Tidak dapat menghubungi layanan login.'}}};
