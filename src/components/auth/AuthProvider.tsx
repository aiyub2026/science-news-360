...
export function AuthProvider({children}:{children:React.ReactNode}){
 ...
 const login=async(email:string,password:string,requestedRole?:Role)=>{try{const adminOnly=requestedRole==='ADMINISTRATOR';const r=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password,adminOnly})});const d=await r.json();if(!r.ok)return {ok:false,error:d.error||'Login gagal.'};const effectiveRoles:Role[]=d.user.roles?.length?d.user.roles:[d.user.role];if(adminOnly&&!effectiveRoles.some((r:Role)=>['ADMINISTRATOR','SYSTEM_ADMINISTRATOR'].includes(r))){return {ok:false,error:'Anda tidak diizinkan mengakses area Administrator.'}}setUser(d.user);await initializePersistence();const redirect=adminOnly?'/dashboard/admin':d.user.role==='SYSTEM_ADMINISTRATOR'?'/dashboard/author':roleDashboard[d.user.role as Role];return {ok:true,redirect}}catch{return {ok:false,error:'Tidak dapat menghubungi layanan login.'}}};
