'use client';
const safeUUID=()=>{try{if(typeof crypto!=='undefined'&&typeof crypto.randomUUID==='function')return crypto.randomUUID()}catch{}return `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`};
...
export function saveMedia(media:MediaMeta){const rows=listMedia();const row={...media,id:media.id||safeUUID(),createdAt:media.createdAt||new Date().toISOString()};const i=rows.findIndex(x=>x.id===row.id);if(i>=0)rows[i]=row;else rows.unshift(row);safeWrite(MEDIA,rows.slice(0,80));return row}
export function removeMedia(id:string){safeWrite(MEDIA,listMedia().filter(x=>x.id!==id))}
export function audit(contentId:string,action:string,detail:string,meta:Partial<CmsAudit>={}){safeWrite(AUDIT,[{id:safeUUID(),contentId,action,detail,at:new Date().toISOString(),...meta},...listAudit()].slice(0,500))}
