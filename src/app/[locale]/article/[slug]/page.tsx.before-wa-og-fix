...
export async function generateMetadata({params}:{params:Promise<{locale:string;slug:string}>}):Promise<Metadata>{
 ...
 const canonical=article?.canonicalUrl||canonicalArticleUrl(lang,slug);const image=article?.socialImageUrl||socialImageUrl(lang,slug);const publishedAt=article?.publishedAt;const modifiedAt=article?.updatedAt||publishedAt;const tags=article?.tags||[];
 return {metadataBase:new URL(getSiteUrl()),title:article?`${title} | Science News 360`:'Science News 360',description,alternates:{canonical,languages:{id:canonicalArticleUrl('id',slug),en:canonicalArticleUrl('en',slug)}},robots:article?{index:true,follow:true,'max-image-preview':'large'}:{index:false,follow:false},openGraph:article?{type:'article',siteName:SITE_NAME,title,description,url:canonical,locale:lang==='id'?'id_ID':'en_US',images:[{url:image,secureUrl:image,width:1200,height:630,alt:article.socialImageAlt||title}],publishedTime:publishedAt,modifiedTime:modifiedAt,authors:[article.author],section:article.category,tags}:undefined,twitter:article?{card:'summary_large_image',site:'@ScienceNews360',creator:'@ScienceNews360',title,description,images:[{url:image,alt:article.socialImageAlt||title}]}:undefined};
}

export default async function ArticlePage({params}:{params:Promise<{locale:string;slug:string}>}){
 const {locale,slug}=await params;const lang=locale==='en'?'en':'id';const article=await getPublishedArticle(lang,slug);if(!article)notFound();const all=(await listPublishedArticles()).filter(x=>x.locale===lang&&x.id!==article.id);const score=(x:typeof article)=>Number(x.category===article.category)*4+(x.tags||[]).filter(t=>(article.tags||[]).includes(t)).length*2;const related=all.sort((a,b)=>score(b)-score(a)).slice(0,2);const jsonLd={'@context':'https://schema.org','@type':article.type==='SCIENCE_NEWS'?'NewsArticle':'Article',headline:article.title,description:article.summary,image:[article.socialImageUrl],datePublished:article.publishedAt,dateModified:article.updatedAt,mainEntityOfPage:article.canonicalUrl,author:{'@type':'Person',name:article.author,affiliation:article.authorInstitution},publisher:{'@type':'Organization',name:'Science News 360',logo:{'@type':'ImageObject',url:absoluteUrl('/images/social-default-1200x630.webp')}}};return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd)}}/><ServerPublishedArticle article={article} related={related}/></>;
}
